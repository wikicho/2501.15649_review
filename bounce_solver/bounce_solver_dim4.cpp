...
double BounceSolverS4::find_top() const {
    ...
    return *std::max_element(
        ...
        [&](double a, double b) { return V_(a) < V_(b); });
}
...
double BounceSolverS4::calculate_qd_(double phi0_) const {
    ...
    double Vt3_phiT_ = 3 * std::pow(phiT_, 2) * V0 / std::pow(phi0_, 2) - 2 * std::pow(phiT_, 3) * V0 /
                       std::pow(phi0_, 3) - 3 * std::pow(phiT_, 2) * V0p / (4 * phi0_) + 3 * std::pow(phiT_, 3) * V0p /
                       (4 * std::pow(phi0_, 2));
    // first derivative of the cubic term
    double Vt3_prime_phiT_ = 6 * V0 * phiT_ / (phi0_ * phi0_) - 6 * phiT_ * phiT_ * V0 / std::pow(phi0_, 3) - 3 * phiT_
                             * V0p / (2 * phi0_) + 9 * V0p * phiT_ * phiT_ / (4 * phi0_ * phi0_);
    // second derivative of the cubic term
    double Vt3_2prime_phiT_ = 6 * V0 / (phi0_ * phi0_) - 12 * phiT_ * V0 / std::pow(phi0_, 3) - 3 * V0p / (2 * phi0_) +
                              9 * V0p * phiT_ / (2 * phi0_ * phi0_);

    double VT = V_(phiT_) - V_(phi_metamin_);

    double ad = 4 * Q_T_Prime * Q_T_Prime - 6 * Q_T * Q_T_2Prime;
    double bd = 3 * Q_T_2Prime * VT - 3 * Q_T_2Prime * Vt3_phiT_ + 4 * Q_T_Prime * Vt3_prime_phiT_ - 3 * Q_T *
                Vt3_2prime_phiT_;
...
double BounceSolverS4::V_t(double phi, double phi0_, double qd_) const {
    double V0 = V_(phi0_) - V_(phi_metamin_);
    double V0p = dV_(phi0_);

    double vt3 = -2 * std::pow(phi, 3) * V0 / std::pow(phi0_, 3) + 3 * std::pow(phi, 2) * V0 / std::pow(phi0_, 2) + 3 *
                 std::pow(phi, 3) * V0p / (4 * std::pow(phi0_, 2)) - 3 * std::pow(phi, 2) * V0p / (4 * phi0_);
...
double BounceSolverS4::V_t_prime(double phi, double phi0_, double qd_) const {
    double V0 = V_(phi0_) - V_(phi_metamin_);
    double V0p = dV_(phi0_);

    double vt3_prime = -6 * phi * phi * V0 / std::pow(phi0_, 3) + 6 * V0 * phi / (phi0_ * phi0_) + 9 * phi * phi * V0p /
                       (4 * phi0_ * phi0_) - 3 * phi * V0p / (2 * phi0_);
    double vt4_prime = 2.0 * qd_ * phi * (2.0 * phi * phi - 3.0 * phi * phi0_ + phi0_ * phi0_);

    return vt3_prime + vt4_prime;
}

double BounceSolverS4::calculate_action(double phi0_) const {
    const double front_factor = 54 * M_PI * M_PI;

    // integrand definition
    std::vector<double> phi_vec;
    std::vector<double> integrand;

    double qd = calculate_qd_(phi0_);

    // numerical integration
    double integral = 0;
    for (int i = 0; i + 1 < V_vec_.size(); ++i) {
        const double dx = V_vec_[i + 1] - V_vec_[i];
        const double height = 0.5 * (std::pow(V_vec_[i + 1] - V_t(phi_vec_[i + 1], phi0_, qd), 2) /
                                     std::abs(std::pow(V_t_prime(phi_vec_[i + 1], phi0_, qd), 3)) + std::pow(
                                         V_vec_[i] - V_t(phi_vec_[i], phi0_, qd), 2) / std::abs(
                                         std::pow(V_t_prime(phi_vec_[i], phi0_, qd), 3)));
        integral += dx * height;
    }

    return front_factor * integral;
}
